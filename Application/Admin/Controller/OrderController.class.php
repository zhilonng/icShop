...
class OrderController extends CommonController{
	...
	public function index(){
        ...
        $designer=$_GET['order_designer'];
        $paper=$_GET['order_paper'];
		...
		if($_GET){
			...
			if($designer){
				$where['order_designer'] =$_GET['order_designer'];
			}
			if($order_style_number){
				$where['order_style_number'] =array('like',"$_GET[order_style_number]") ;
			}
            if($factory){
                $where['order_factory']=array('like',"$_GET[factory_id]") ;
            }
            if($paper){
                $where['order_paper']=$_GET['order_paper'];
            }
            if($cate){
    ...
    //动态的修改详情单里面的数据
    {
    	if(IS_POST){  
                ...
                $filepath=M('order')->where("order_id=$id")->getfield('order_pic');
                if($_POST['imgpath']){
                ...
                $list=M('Order')->find($order_id);
                 $factoryarr=explode(',', $list['order_factory']);
                foreach ($factoryarr as  $vi) {
                    $list['factory_name'][]=M('factory')->where("factory_id=$vi")->getfield('factory_name');
                }
                $list['factory_name']=implode(',', $list['factory_name']);
                $this->list=$list;
                ...
                foreach ($did as $ko => $vo) {
                    ...
                    $rdata=M('Redo')->where("redo_detail_id=$d_did")->field('redo_id,redo_detail_total')->select();
                    $num=0;
    ...
    public function delDetail()
    {
        ...
        $id=I('oid');
        $detail_id=I('did');
        if(M('Detail')->delete($did)){
            $str=M('Order')->where("order_id=$id")->getfield('order_detail_id');
            $data['order_id']=$id;
            $data['order_detail_id']=str_replace($did,'',$str);
            if(M('Order')->save($data)){
                M('Redo')->where("redo_detail_id=$detail_id")->delete();
                echo 1;
            }else{
                echo 0;
            }
    ...
    {
        $id=I('id');
        $old=M('detail')->where("detail_id=$id")->getfield('detail_total');
        if($old==I('total')){
            die('2');
        }
        $data['detail_id']=I('id');
        $data['detail_total']=I('total');
        if(M('Detail')->save($data)!==false){
            echo 1;
        }else{
            echo 0;
        }
     ...
     public function view()
    {
                $order_id=$_GET['order_id'];
                $factory=M('factory')->select();
                $list=D('Order')->relation(true)->find($order_id);
                //p($list);
                $factoryarr=explode(',', $list['order_factory']);
                foreach ($factoryarr as  $vi) {
                    $list['factory_name'][]=M('factory')->where("factory_id=$vi")->getfield('factory_name');
    	...
    	if(IS_POST){
    		//p($_POST);
                ...
                foreach ($d_data as $ko => $vo) {
                    $number[]=M('detail')->add($d_data[$ko]);
                }
                    $data['order_pic']=I('imgpath'); 
                    $data['order_detail_id']=implode($number,',');
                    $data['order_factory']=I('order_factory') ;
                    $data['order_style_number']=I('order_style_number') ;
                    $data['order_designer']=I('order_designer');
                    $data['order_paper']=I('order_paper');
                    $data['order_category']=I('order_category') ;
                    $data['order_fmoney']=I('order_fmoney');
                    $data['order_mfoney']=I('order_mfoney');
                    ...
                    $data['order_remarks']=I('order_remarks');
    ...
    public function dateDetail($order_id){
        ...
        $arrdetail=M('Order')
        ->field('b_order.*,f.factory_name')
        ->join('__FACTORY__ as f on __ORDER__.order_factory=f.factory_id')       
        ->find($order_id);
        ...
        foreach($arrdetail as$k=> $v){
            ...
            $Product = D('Product');
            $data[] = $Product
            ->field('b_product.*,d.detail_color,d.detail_size,d.detail_total,d.detail_alltotal,c.color_name,s.size_name')
            ->join('LEFT JOIN __DETAIL__ as d on __PRODUCT__.product_detail_id=d.detail_id')
    ...
    public function del($order_id)
    {
        ...
        //p($detail_ids);
        if(M('Redo')->where("redo_order_id=$order_id")->delete()===false){
            $this->error('删除失败');
            die;
        }
        foreach ($detail_ids as $k => $v) {
            if(M('Detail')->delete($v)){
                continue;
            }
        }
        if(M('Product')->where("product_order_id=$order_id")->delete()===false){
            $this->error('删除失败');
            die;
        }
        if(M('Order')->delete($order_id)){
...
";
    ...
    public function addRedo()
    {
        ...
        if(IS_POST){
            ...
            $factoryarr=explode(',', M('Order')->where(array('order_id'=>$order_id))->getfield('order_factory'));
            if(!in_array($order_factory, $factoryarr)){
                $factoryarr[]=$order_factory;
                $odata['order_factory']=implode(',',$factoryarr);
                $odata['order_id']=$order_id;
                if(M('Order')->save($odata)===false){
            ...
            foreach ($d_data as $ko => $vo) {
                if(!$vo['detail_id']){
                    ...
                    if($vo['detail_id']=M('detail')->add($newdata)){
            ...
            //p($detail_arr);
                $order_id=I('order_id');
                $list_str=M('Order')->where(array('order_id'=>$order_id))->getfield('order_detail_id');
                ...
                foreach($list_str as $v){
                    $newtotal+=M('Detail')->where(array('detail_id'=>$v))->getfield('detail_total');
                }
                ...
                if(M('Order')->save($odata)!==false){
                    ...
                    foreach ($sdata as $key => $value) {
                        ...
                        $this->success('返单添加成功',U('Admin/order/index'));
                }else{
    ...
    public function delRedo()
    {
        ...
        $dtotal=M('detail')->where("detail_id=$detail_id")->getfield('detail_total');
        //返单的数量
        $redo_total=$list['redo_detail_total'];
        if($dtotal==$list['redo_detail_total']){
            //p(11);
            if(M('detail')->delete($detail_id)){
                //删除订单里详情记录id记录
                $lists=M('Order')->field('order_total,order_detail_id')->find($order_id);
            ...
            $ototal=M('Order')->where("order_id=$order_id")->getfield('order_total');
            $odata['order_id']=$order_id;
    ...
    {
        $list=M('Redo')
        ->join('LEFT JOIN __FACTORY__ on __FACTORY__.factory_id=b_redo.redo_factory')
        ...
        $odata=M('Order')
        ->field('b_order.order_style_number')
        ->find($order_id);
        $this->odata=$odata;
        $this->factory=M('factory')->select();
        $this->list=$list;
    ...
    {
        $no=I('no');
        if(M('order')->where("order_style_number='$no'")->find()){
            echo 'repeat';
        }else{
            echo 'no';
        }
