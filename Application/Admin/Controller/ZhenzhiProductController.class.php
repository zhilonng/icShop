...
class ZhenzhiProductController extends CommonController{
	...
	public function index(){
		...
		if($_GET){
			...
			if($style_num){
				$oid=M('ZhenzhiOrder')->where("order_style_number='$style_num'")->getfield('order_id');
				$where['product_order_id']=$oid;
			}
            if($factory){
                $where['b_zhenzhi_order.order_factory']=array('like',"%$_GET[factory_id]%") ;
			}
			if($_GET['today']==1){
		...
		$Product = M('ZhenzhiProduct');
		$count = $Product
		->where($where)
		->field('b_zhenzhi_product.*,b_zhenzhi_order.order_factory')
		->join('LEFT JOIN b_zhenzhi_order  on b_zhenzhi_product.product_order_id=b_zhenzhi_order.order_id')
		->count();
		//p($count);
		$Page = new \Think\Page($count,20);
		$show = $Page->show();
		$list = $Product
		->field('b_zhenzhi_product.*,b_zhenzhi_order.order_factory,b_zhenzhi_order.order_style_number,b_zhenzhi_order.order_date,b_zhenzhi_order.order_fmoney,b_zhenzhi_order.order_allmoney')
		->join('LEFT JOIN b_zhenzhi_order on b_zhenzhi_product.product_order_id=b_zhenzhi_order.order_id')
		->where($where)
		//->relation(true)
		...
		foreach ($list as $k => $v) {
			...
			$data=D('ZhenzhiDetail')->relation(true)->find($did);
			//p($data);
	...
	public function add(){
		if(IS_POST){
			...
			foreach ($ndata as $key => $value) {
				...
				if(M('ZhenzhiDetail')->save($ddata)!==false){
					if(M('ZhenzhiProduct')->add($data)){
						//三表操作完成
					}else{
                         $error=M('ZhenzhiProduct')->getDbError();
					}
				}else{
				  	$error=M('ZhenzhiDetail')->getDbError();				
				}
			}
			//开始处理事务
	        if($error){
	            ...
	            $order->commit();
	            $this->success('出货记录添加成功',U('Admin/ZhenzhiProduct/add'));
	        }
		}else{
			//p($_SESSION);
	...
	{
		$style_id=I('style_id');
        $list=M('ZhenzhiOrder')->field('order_detail_id,order_id')->where("order_style_number='$style_id'")->find();
        $oid=$list['order_id'];
	...
	public function edit(){
		if(IS_POST){
			...
			$all_total=M('ZhenzhiDetail')->where("detail_id=$d_id")->getfield('detail_alltotal');
			...
			if(M('ZhenzhiDetail')->save($ddata)!==false){
        		if(M('ZhenzhiProduct')->save($pdata)!==false){
        			$this->success('编辑成功', U('ZhenzhiProduct/index'));
        		}else{
					$this->error('编辑失败');
			    }
			}else{
			...
			$id = I('product_id');
			$list=D('ZhenzhiProduct')->relation(true)->find($id);
			$did=$list['product_detail_id'];
			$data=D('ZhenzhiDetail')
			->relation(true)
			->find($did);
	...
	public function del(){
		...
		$pid = I('product_id');
		$_result = M('ZhenzhiProduct')->where("product_id=$pid")->find($pid);
		$did=$_result['product_detail_id'];
		$detail_alltotal=M('ZhenzhiDetail')->where("detail_id=$did")->getfield('detail_alltotal');
		$ddata['detail_alltotal']=$detail_alltotal-$_result['product_quantity'];
		//p($ddata['detail_alltotal']);
		$ddata['detail_id']=$did;
		if(M('ZhenzhiDetail')->save($ddata)!==false){
            if(M('ZhenzhiProduct')->delete($pid)){
				$this->success('删除成功', U('ZhenzhiProduct/index'));            
			}else{
				$this->error('删除失败');
			}
		}else{
